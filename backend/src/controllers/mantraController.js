  ...
  try {
    db.get(sql, [displayTime], (error, rows) => {
      if (error) {
        return res
          .status(500)
          .json({ msg: "Erro interno no servidor ", error });
      }
    ...
    return res.status(500).json({
      msg: `erro ao buscar os mantras pelo displayT: ${error.message}`,
    });
  }
}
...
function searchMantrasByplayOnStartup(req, res) {
  ...
  try {
    ...
    console.log(
      `erro ao buscar os mantras pelo playOnStartup: ${error.message}`
    );
    return res.status(500).json({
      msg: `erro ao buscar os mantras pelo playOnStartup: ${error.message}`,
    });
  }
}
