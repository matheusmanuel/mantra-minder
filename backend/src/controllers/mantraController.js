...
function updateMantra(req, res) {
  try{
    ...
    db.run(sql, [mantra.mantraTitle, mantra.mantraText, mantra.isActive, mantra.playOnStartup, mantra.displayTime, mantra.mantraID], async function(error){
      if(error){
        res.status(500).json({msg: `[sql]Houve um erro ao editar o mantra: ${error.message}`});
        console.log("[sql]Houve um erro ao editar o mantra cmd: ", error.message);
        return false;
      }else{
        res.status(200).json({msg:`O mantra com id: ${mantra.mantraID} foi editado com sucesso`});
        console.log(`O mantra com id: ${mantra.mantraID} foi editado com sucesso`);
      }
    })
  }catch(error){
